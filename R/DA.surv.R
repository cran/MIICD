#'  @title Survival estimation for interval censored data using data augmentation and multiple imputation
#'  @author Marc Delord \email{<mdelord@@gmail.com>}
#'  @description Uses data augmentation and multiple imputation to compute the survival function with interval censored data
#'  @inheritParams DA.ci
#'  @examples
...
#'  @details This function uses data augmentation and multiple imputation aproach to estimate the survival function interval censored
#'  data.
#'  Estimates are computed using Rubin's rules (Rubin (1987)). The survival is computed as the mean of survival over imputations. The
...
DA.surv<-function(k, m, data, conf.int = FALSE, alpha = 0.05){
...
if(k <= 1) stop('You may consider the MI.surv function')  
...
#' plot method for DA_surv objects
#' @param x a DA_surv object
#' @param xlab Label for x axis
#' @param ylab Label for y axix
#' @param fun If fun = event, 1 - the surv is drown
#' @param ... other arguments
#' @export 
plot.DA_surv <- function (x , xlab = 'Time' , ylab = 'Survival' , fun = c('surv','event') , ... )
      {
      fun<-match.arg(fun)    
      data <- x$est
      conf.int <- x$conf.int
